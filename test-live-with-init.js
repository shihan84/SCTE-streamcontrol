/**
 * Live HLS Stream SCTE-35 Testing with Initialization
 * 
 * Tests real SCTE-35 injection using the provided HLS stream:
 * Input: https://cloud.itassist.one/khaber24x7live/index.m3u8
 * 
 * This script initializes the streaming components first, then tests all formats
 * 
...
async function initializeStreaming() {
    console.log('\n🚀 Initializing streaming components...');
...
function generateTestReport() {
    ...
    const reportPath = `./live-test-results-${Date.now()}.json`;
    require('fs').writeFileSync(reportPath, JSON.stringify(testResults, null, 2));
    console.log(`\n💾 Detailed test report saved to: ${reportPath}`);
    
    console.log('\n🎉 Live SCTE-35 testing completed!');
}

// Main test function
async function runLiveTests() {
    console.log('🎬 Live HLS Stream SCTE-35 Testing with Initialization');
    console.log('========================================================');
    ...
    // Initialize streaming components
    const initialized = await initializeStreaming();
    if (!initialized) {
        console.log('❌ Cannot proceed - failed to initialize streaming components');
        return;
    }
