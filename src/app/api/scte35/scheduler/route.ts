...
async function executeSchedule(schedule: AdSchedule, execution: ScheduleExecution): Promise<ScheduleExecution> {
  try {
    // This would integrate with the self-hosted media server SCTE-35 API
    // For now, we'll simulate the execution
    
    const serverUrl = 'http://localhost:8080' // This should come from configuration
    const username = 'admin' // This should come from configuration
    const password = 'password' // This should come from configuration

    // Simulate API call to self-hosted media server
    const response = await fetch(`${serverUrl}/api/media-server/scte35/inject`, {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        pre_roll: schedule.preRoll,
        upid: schedule.metadata.upid,
        segmentation_type_id: schedule.metadata.segmentationTypeId,
        segmentation_message: schedule.metadata.segmentationMessage
      }),
    ...
    return {
      ...
      result: {
        success,
        eventId: success ? `evt_${Date.now()}` : undefined,
        error: success ? undefined : 'Failed to trigger SCTE-35 event'
      }
    }
  } catch (error) {
    return {
  ...
  try {
    ...
    switch (action) {
      ...
      case 'trigger_now':
        ...
        const restrictionCheck = checkRestrictions(triggerSchedule)
        if (!restrictionCheck.allowed) {
          return NextResponse.json(
            { success: false, error: restrictionCheck.reason },
            { status: 400 }
          )
        }
        ...
        const result = await executeSchedule(triggerSchedule, execution)

        // Update execution record
        const execIndex = scheduleExecutions.findIndex(e => e.id === execution.id)
        if (execIndex !== -1) {
          scheduleExecutions[execIndex] = result
        }
        ...
        return NextResponse.json({
          ...
          data: {
            execution: result,
            message: result.result?.success ? 'Schedule triggered successfully' : 'Failed to trigger schedule'
          },
          timestamp: new Date().toISOString()
        })

      case 'get_executions':
        const execScheduleId = scheduleData.scheduleId
        const execLimit = scheduleData.limit || 50
        const execOffset = scheduleData.offset || 0

        const executions = scheduleExecutions
          .filter(e => execScheduleId ? e.scheduleId === execScheduleId : true)
          .sort((a, b) => new Date(b.scheduledTime).getTime() - new Date(a.scheduledTime).getTime())
          .slice(execOffset, execOffset + execLimit)

        return NextResponse.json({
          success: true,
          data: {
            executions,
            total: scheduleExecutions.length,
            limit: execLimit,
            offset: execOffset,
            timestamp: new Date().toISOString()
          }
    ...
    console.error('Error executing ad schedule action:', error)
    return NextResponse.json(
      { success: false, error: 'Failed to execute ad schedule action' },
      { status: 500 }
    )
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const scheduleIndex = adSchedules.findIndex(s => s.id === scheduleId)
    if (scheduleIndex === -1) {
      return NextResponse.json(
        { success: false, error: 'Schedule not found' },
        { status: 404 }
    ...
    adSchedules.splice(scheduleIndex, 1)
    ...
    return NextResponse.json(
      { success: false, error: 'Failed to delete ad schedule' },
      { status: 500 }
    )
  }
}